...
def fun(sets,n,p,ratio,param,null_type,kernel_type):
    ...
    #generate data
    X, Y = dgpK3_unb(n, p, ratio, param, null_type)
...
#     output_dir = "Z:/User/Documents/pudan/kernel/20241010/case3"
#     ns=[100,150,200,250,300,350]
#     ps=[200]
#     thetas=[0.35,0.35,0.2,4]
#     types=["mean","cov","loc","dstr"]
#     kernel_types=["RBF","Laplace","Laplace2"]
    
#     merge = pd.DataFrame(columns=["kernel_type","type","n",
...
#                         res = pool.starmap(fun, [(sets, ns[i], ps[0], 1/2, thetas[k], 
#                                                   types[k],kernel_types[l]) for sets in range(1, 201)])
                        
#                     res = pd.DataFrame(res)
#                     res.to_csv(f"{output_dir}/{ns[i]}_{ps[0]}_{thetas[k]}_{types[k]}_{kernel_types[l]}.csv", index=False)
                    
#                     #merge results
#                     merge.loc[len(merge)]=[kernel_types[l],types[k],ns[i]]+list(np.mean(res, axis=0))
