...
def fun(sets,n,p,ratio,param,null_type,kernel_type):
    ...
    #generate data
    X, Y = dgpK_unb(n, p, ratio, param, null_type)
...
#     merge = pd.DataFrame(columns=["kernel_type","type","p",
...
#                         res = pool.starmap(fun, [(sets, ns[i], ps[j], 1/2, thetas[k], 
#                                                   types[k],kernel_types[l]) for sets in range(1, 201)])
                        
#                     res = pd.DataFrame(res)
#                     res.to_csv(f"{output_dir}/{ns[i]}_{ps[j]}_{thetas[k]}_{types[k]}_{kernel_types[l]}.csv", index=False)
                    
#                     #merge results
#                     merge.loc[len(merge)]=[kernel_types[l],types[k],ps[j]]+list(np.mean(res, axis=0))
