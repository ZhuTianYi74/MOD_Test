...
def fun(sets,n,p,ratio,param,null_type,kernel_type):
    ...
    #generate data
    X, Y = dgpK4_unb(n, p, ratio, param, null_type)
...
if __name__ == "__main__":
    output_dir = "Z:/User/Documents/pudan/kernel/20241010/case4"
    ns=[200]
    ps= [200]
    thetas=[0.12,0.12,0.08,7]
    types=["mean","cov","loc","dstr"]
    kernel_types=["RBF","Laplace","Laplace2"]
    ratios=[0.3,0.4,0.5,0.6,0.7,0.8]
    
    merge = pd.DataFrame(columns=["kernel_type","type","ratio",
            ...
            for k in [0,1,2,3]:   
                for j in [0,1,2,3,4,5]:
                    with Pool(processes=min(int(cpu_count() * 0.7),int(60))) as pool:
                        res = pool.starmap(fun, [(sets, ns[0], ps[0], ratios[j], thetas[k], 
                                                  types[k],kernel_types[l]) for sets in range(1, 201)])
                        
                    res = pd.DataFrame(res)
                    res.to_csv(f"{output_dir}/{ns[0]}_{ps[0]}_{ratios[j]}_{thetas[k]}_{types[k]}_{kernel_types[l]}.csv", index=False)
                    
                    #merge results
                    merge.loc[len(merge)]=[kernel_types[l],types[k],ratios[j]]+list(np.nanmean(res, axis=0))
...
#         plt.savefig(f'{output_dir}/{kernel}_plot(size).png')
